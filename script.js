document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('generateAllKeysBtn').addEventListener('click', async () => {
        ...
        generateAllKeysBtn.classList.add('hidden');
    
        let progress = 0;
        const updateProgress = (increment, message) => {
            progress += increment;
            progress = Math.min(progress, 100); // гарантирует, что прогресс не превысит 100%
            progressBar.style.width = `${progress}%`;
            progressText.innerText = `${Math.round(progress)}%`;
            progressLog.innerText = message;
        };
    
        const allKeys = await Promise.all(gameIds.map(async (gameId) => {
            const game = games[gameId];
            const keys = await Promise.all(Array.from({ length: keyCount[gameId] }, async () => {
                const clientId = generateClientId();
                let clientToken;
                try {
                    clientToken = await login(clientId, game.appToken);
                } catch (error) {
                    alert(`Ошибка входа: Failed to login. Превышен лимит запросов, смените IP, включив VPN сервис, или ожидайте 10 минут для обнуления лимитов запросов: ${error.message}`);
                    return null;
                }
    
                for (let i = 0; i < game.attempts; i++) {
                    const hasCode = await emulateProgress(clientToken, game.promoId);
                    updateProgress((100 / game.attempts) / (gameIds.length * keyCount[gameId]), `Эмуляция прогресса ${i + 1}/${game.attempts} для игры ${game.name}...`);
                    if (hasCode) {
                        break;
                    }
                ...
                try {
                    ...
                    updateProgress(100 / (gameIds.length * keyCount[gameId]), `Генерация ключа для игры ${game.name}...`);
                    return key;
                } catch (error) {
                    alert(`Не удалось сгенерировать ключ для игры ${game.name}: ${error.message}`);
                    return null;
                }
            }));
        ...
        const allKeysFlat = allKeys.flat().filter(key => key);
        if (allKeysFlat.length > 1) {
            keysList.innerHTML = allKeysFlat.map(key =>
                `<div class="key-item">
        ...
        } else if (allKeysFlat.length === 1) {
            keysList.innerHTML =
                `<div class="key-item">
                    <input type="text" value="${allKeysFlat[0]}" readonly>
                    <button class="copyKeyBtn" data-key="${allKeysFlat[0]}">Скопировать</button>
                </div>`;
        }
        ...
        copyAllBtn.addEventListener('click', () => {
            const keysText = allKeysFlat.join('\n');
            copyToClipboard(keysText);
        });
    
        progressBar.style.width = '100%';
        progressText.innerText = '100%';
        progressLog.innerText = 'Конец генерации';
    
        document.querySelector('.grid-container').style.display = 'grid';
        keyCountGroup.classList.remove('hidden');
        generateAllKeysBtn.classList.remove('hidden');
    });
...
//-----------------------------------------------------------------
    ...
    startBtn.addEventListener('click', async () => {
        ...
        const updateProgress = (increment, message) => {
            ...
            progress = Math.min(progress, 100); // гарантирует, что прогресс не превысит 100%
            progressBar.style.width = `${progress}%`;
            progressText.innerText = `${Math.round(progress)}%`;  // округление для отображения
            progressLog.innerText = message;
        };
        ...
        const generateKeyProcess = async () => {
            ...
            try {
                const key = await generateKey(clientToken, game.promoId);
                updateProgress(100 / keyCount, 'Генерация ключа...');
                return key;
            } catch (error) {
    ...
    const copyToClipboard = (text) => {
        if (navigator.clipboard && navigator.clipboard.writeText) {
            ...
            try {
                ...
                console.error('Резервный вариант: К сожалению, невозможно скопировать', err);
            }
